...
def WriteCSVFileInternalResistance(MainSubDir,FinalClass,RestTime,LowOnTime,HighOnTime,LowCurrent,HighCurrent):
    print("Got to WriteCSVFileInternalResistance")
    FinalCsvFileObj = open(MainSubDir+'\\FinalIR.csv','w')
    FinalClass.FindInterpolatedCsvValues()
    FinalClass.CalculateValues() # Calculate values
    print('Time [s],Voltage [V],Current [A],Thermo Ave,CJ Ave,Depth Of Discharge [%],R [Ohms],Low Current = '+str(LowCurrent)+',High Current = '+str(HighCurrent)+',LowOnTime = '+str(LowOnTime)+',HighOnTime = '+str(HighOnTime)+',RestTime = '+str(RestTime),file=FinalCsvFileObj)
    for i in range(0,len(FinalClass.FinalTime)-1,1):
        if (i % (RestTime+LowOnTime+HighOnTime)) == 0 and i != 0: # Increment and not equal to 0
            if abs(float(FinalClass.Current[i]) - HighCurrent) < 0.05: # ensure current hasn't started to drop already, if so grab the one before
                Vhigh = float(FinalClass.FinalVoltage[i])
            else:
                Vhigh = float(FinalClass.FinalVoltage[i-1])
            if abs(float(FinalClass.Current[i]) - LowCurrent) < 0.05: # ensure current hasn't started to drop already, if so grab the one before
                Vlow = float(FinalClass.FinalVoltage[i-HighOnTime])
            else:
                Vlow = float(FinalClass.FinalVoltage[i-HighOnTime])
            Ihigh = HighCurrent
            Ilow = LowCurrent
            Rstr = '=('+str(Vlow)+'-'+str(Vhigh)+')/('+str(Ihigh)+'-'+str(Ilow)+')'
            
            print(FinalClass.FinalTime[i]+','+FinalClass.FinalVoltage[i]+','+\
                  FinalClass.FinalCurrent[i]+','+FinalClass.FinalThermo[i]+','+FinalClass.FinalCJ[i]+','+\
                  FinalClass.FinalDoD[i]+','+Rstr,file=FinalCsvFileObj)
        else:
            print(FinalClass.FinalTime[i]+','+FinalClass.FinalVoltage[i]+','+\
                  FinalClass.FinalCurrent[i]+','+FinalClass.FinalThermo[i]+','+FinalClass.FinalCJ[i]+','+\
                  FinalClass.FinalDoD[i],file=FinalCsvFileObj)
...
Path = "C:\\Users\\Alexander\\Desktop\\ELECTRO\\Thermo_Testing\\3 Part Test\\_FIrst\\2019-2-23_8-54-35_Testing_IR"
