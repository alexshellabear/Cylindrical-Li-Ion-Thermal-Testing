...
from OCVEstimateClass import OCVEstimateClass # Entropy testing
        ...
        if type(CBAExecutableFileLocation) == str:
            if type(BaseDirectory) == str:
                if type(MaxTemperature) == int:
                    ...
                    CurrentDirectory = BaseDirectory+os.sep+DateTimeStamp+"_Testing_OCVEstimate"
                    MakeDirectory(CurrentDirectory)
                    
                    # Do the testing
                    OCVEstimateClass(CBAExecutableFileLocation,CurrentDirectory,MaxTemperature)
                else:
